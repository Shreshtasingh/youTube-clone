...
const createChannel = async (req, res) => {
  try {
    const channel = req.channel; //fetched the name of channel
    const uid = generateID(channel.id); //generated id
    ...
    if (
      ...
      req.query.handle !== req.channel.uid //handle already exists which is not the same as the current channel's handle
    ) {
      return res.status(400).json({ message: "Channel handle already exists" });
    }
    //if handle is verified and channel being created successfully
    ...
    res
      .status(500)
      .json({
        error: "Oops! Something went wrong while creating the channel.",
      });
  }
};
const updateChannel = async (req, res) => {
  try {
    ...
    if (req.files?.banner) { //for banner
      const response = await imageKit.upload({
...
const subscribeChannel = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Subscription successful! Welcome to the club 🎉" });
  } catch (error) {
    console.error("Subscription error:", error);
    res
      .status(500)
      .json({ error: "Oops! Something went wrong while subscribing." });
  }
};
...
const unsubscribeChannel = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Oops! Something went wrong while unsubscribing." });
  }
};
...
const notificationsChannel = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Oops! Something went wrong while setting notifications.",
      });
  }
};
module.exports = {
